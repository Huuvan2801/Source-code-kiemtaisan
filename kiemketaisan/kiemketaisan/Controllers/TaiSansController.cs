...
namespace kiemketaisan.Controllers
{
    public class TaiSansController : Controller
    {
        ...
        // GET: TaiSans
        public ActionResult Index(string search="",int? IdPB=0)
        {
            if (Session["Id"] == null)
            {
            ...
            ViewBag.tenphong = "";
            ViewBag.IdPB = IdPB;
            var PQ = Convert.ToInt32(Session["PQ"]);
            var IdTK = Convert.ToInt32(Session["Id"]);
            if (PQ != 1)
            {
                IdPB=db.ThongTins.FirstOrDefault(g=>g.IdTK== IdTK).IdPB;
            }
            var list = db.TaiSans.AsQueryable();
            if (IdPB > 0)
            {
                ViewBag.tenphong = db.PhongBans.Find(IdPB).TenPhongBan;
                list=list.Where(g=>g.IdPB == IdPB);
            }
            ViewBag.IdPB = IdPB;
            if (!string.IsNullOrEmpty(search))
            {
                list=list.Where(g=>g.TenTS.Contains(search));
            }
        ...
        // GET: TaiSans/Create
        public ActionResult Create(int? IdPB=0)
        {
            ViewBag.IdPB = IdPB;
        ...
        public ActionResult Create([Bind(Include = "Id,TenTS,NoiSuDung,NamDVSD,SoKiemKeTT,SoTheoKeToan,NguyenNhan,Gia,TinhTrang,GhiChu,IdPB")] TaiSan taiSan)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    var IdTK = Convert.ToInt32(Session["Id"]);
                    taiSan.IdPB = db.ThongTins.FirstOrDefault(g => g.IdTK == IdTK).IdPB;
                }
                db.TaiSans.Add(taiSan);
                db.SaveChanges();
                return RedirectToAction("Index", "TaiSans", new {IdPB=taiSan.IdPB});
            }
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "Id,TenTS,NoiSuDung,NamDVSD,SoKiemKeTT,SoTheoKeToan,NguyenNhan,Gia,TinhTrang,GhiChu,IdPB")] TaiSan taiSan)
        {
            if (ModelState.IsValid)
            {
